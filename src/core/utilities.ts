...
export const Utilities = {
	...
	formula: (toEval: string, parameters: Record<string, string | number>) => {
		if (parameters) {
			parameters = (() => {
				const neoParameters: Record<string, string | number> = {}
	...
	excludeAttrs(attrs: TRecord, ...names: string[]) {
		return () => {
			const response: TRecord = {}
			for (const name in attrs) {
				if (!names.includes(name)) response[name] = attrs[name]
			}
			return response
		}
	}
